...
void Tim::InterruptSwitch(State state, ITMode it)
{
    //使用库函数配置
...
/// @brief 获取一个默认配置的计数器初始化结构体指针
/// @return 默认的计数器初始化结构体指针
...
void Tim::SetClockMode(TIMClockMode mode, InternalTrigger trigger, ExternalPrescaler extPrescaler, TIxChannel tixChannel, Polarity polarity, uint16_t tixFilter)
{
    if (mode == TIMClockMode::InternalClock)
    {
        TIM_InternalClockConfig(self);
    }
    else if (mode == TIMClockMode::OtherTIM)
    {
        TIM_ITRxExternalClockConfig(self, static_cast<uint16_t>(trigger));
    }
    else if (mode == TIMClockMode::TIxClock)
    {
        TIM_TIxExternalClockConfig(self, static_cast<uint16_t>(tixChannel), static_cast<uint16_t>(polarity), tixFilter);
    }
    else if (mode == TIMClockMode::ExternalClockMode1)
    {
        if (polarity == Polarity::Rising)
        {
    ...
    else if (mode == TIMClockMode::ExternalClockMode2)
    {
        if (polarity == Polarity::Rising)
        {
...
void Tim::SetPrescaler(uint16_t prescaler, PSCReloadMode reloadMode)
{
...
/// @brief 设置计时器计数模式
/// @param mode 修改的目标模式
void Tim::SetCounterMode(CounterMode mode)
{
...
/// @param flag 要获取的标志位
/// @return 标志位的状态
FlagStatus Tim::GetFlagStatus(Flags flag)
{
...
/// @param it 要获取的中断模式
/// @return 中断状态
ITStatus Tim::GetITStatus(ITMode it)
{
...
/// @brief 清除指定的中断挂起位
/// @param it 要清除的中断模式
void Tim::ClearITPendingBit(ITMode it)
{
